...
* Die theoretischen Grundlagen zu diesen Übungen findest du unter [06 Korrelation](https://rpubs.com/stammler/810014).
...
c) Alter zwischen den Ehepartnern korreliert stärker (r = 939) als Körpergrösse (r = 0.306). Die Punkte im Altersplot streuen weniger um eine gedachte Gerade als die Punkte im Plot zur Körpergrösse.   
<br>
...
Welcher Korrelationskoeffizient passt zu welcher Grafik?
...
Besteht ein Zusammenhang zwischen Körpergrösse und Schuhgrösse bei den Physiotherapie-Studierenden.

0. Laden Sie den Datensatz `phydata.csv` bzw. `phydata.omv` in jamovi.   
...
* $H_0$: Es besteht kein Zusammenhang zwischen Schuhgrösse und Körpergewicht.    
* $H_A$: Es besteht ein Zusammenhang zwischen Schuhgrösse und Körpergewicht.
<br />
...
```{r, eval=TRUE}
phy <- read_csv("../Datasets/phydata.csv")
...
# data(anscombe)
# summary(anscombe)
# write_csv(anscombe, file = "anscombe.csv")
anscombe <- read_csv("../Datasets/anscombe.csv")
```
...
0. Laden Sie den Datensatz `anscombe.csv`. Download [hier](https://1drv.ms/u/s!AgeAm23J8ZSmi-xFGwGSXZw3ZfW0qw?e=Ljlkgk). Sie müssen die x-Variablen als *continuous* kategorisieren.          
...
2. Berechnen Sie die Korrelationskoeffizienten nach Pearson für die 4 x-y-Paare.   
...
* Bis auf die 4. Stelle nach dem Komma sind alle Korrelationskoeffizienten gleich und weisen auf einen starken Zusammenhang hin.  
...
# 
# write_csv(anscombe_m, file = "anscombe_m.csv")
anscombe_m <- read_csv("../Datasets/anscombe_m.csv")
...
# anscombe_x <- anscombe_m %>% 
#   group_by(set) %>% 
#   mutate(row_nr = row_number()) %>% 
#   pivot_wider(names_from = set, values_from = c(x, y), names_sep = ".") %>% 
#   select(-row_nr)
# 
# cor(anscombe_x)
# cor(anscombe_x, method = "spearman")
```
...
```{r, eval=TRUE}
# library(readr)
# calories <- read_csv("C:/Users/balbu/switchdrive/Beruf/R/R_Projects/SG_PHY_StatExercises/PHY19/calories.csv")
# calories5000 <- calories %>% 
#   sample_n(5000, replace = FALSE)
# write_csv(calories5000, file = "calories5000.csv")

calories <- read_csv("../Datasets/calories5000.csv")
...
Wir arbeiten mit dem Datensatz `calories5000.csv` eines Fitness-Tracker-Anbieters. Download [hier](https://1drv.ms/u/s!AgeAm23J8ZSmi-xICkhvEY0P9gWqBw?e=lUMZcr)
...
cor.test(data = calories, ~ Heart_Rate + Calories)
cor.test(data = calories, ~ Heart_Rate + Calories, method = "spearman")
```

**Es besteht ein signifikanter positiver Zusammenhang zwischen Herzfrequenz und Kalorienverbrauch, $p$ = .90, $r_S$ = .92, p < .0001. Auf Grund der Grafik ist ein vollkommen linearer Zusammenhang unsicher. Für $p$ ist ein linearer Zusammenhang voraussetzung, deshalb entscheiden wir uns für den $r_s$.**
<br />
...
cor.test(data = calories, ~ Body_Temp + Calories)
cor.test(data = calories, ~ Body_Temp + Calories, method = "spearman")
``` 

**Es besteht ein signifikanter positiver Zusammenhang zwischen Körpertemperatur und Kalorienverbrauch, $p$ = .82, $r_s$ = .92, p < .0001. Der Zusammenhang ist eindeutig nicht linear und wir entscheiden uns für $r_s$.**    
<br />
...
cor.test(data = calories, ~ Heart_Rate + Body_Temp)
cor.test(data = calories, ~ Heart_Rate + Body_Temp, method = "spearman")
```
  
**Es besteht ein signifikanter positiver Zusammenhang zwischen Körpertemperatur und Herzfrequenz, $p$ = .78, $r_s$ = .81, p < .0001. Der Zusammenhang ist eindeutig nicht linear und wir entscheiden uns für $r_s$.**    
<br />  
...
cor.test(data = calories, ~ Age + Duration)
cor.test(data = calories, ~ Age + Duration, method = "spearman")
```   

   
**Es besteht kein Zusammenhang zwischen Alter und Trainingsdauer, $p$ = .04, p = .0095; $r_s$ = .03, p = .0346.** 
<br />
<br />
...
cor.test(data = calories, ~ Intensity + Calories)
cor.test(data = calories, ~ Intensity + Calories, method = "spearman")
```

**Es besteht ein signifikanter, linearer und postiver Zusammenhang zwischen Trainingsintensität und Kalorienverbrauch. Mit $p$ = .9743 [.9729, .9757], p < .0001 ist der Zusammenhang sehr stark.**
<br />
...
```{r}
m100 <- read_delim("../Datasets/m100.csv", ";", escape_double = FALSE, trim_ws = TRUE)

ggplot(m100, aes(x = ALTER_jahre, y = ZEIT_sec)) +
  geom_point(col = COL[1], size = 2, alpha = .7)
...
corruption <- read_delim("../Datasets/corruption.csv", ";", escape_double = FALSE, trim_ws = TRUE)
...
ggplot(corruption, aes(x = Median_BMI, y = Corruption_Perceptions_Index_2017)) +
  geom_point(color = "red", size = 2, alpha = .7) +
  geom_smooth(method = "lm", se = FALSE) +
  geom_text(aes(label = Country), check_overlap = TRUE)

cor.test(data = corruption, ~ Median_BMI + Corruption_Perceptions_Index_2017)
```
3. Interpretieren Sie ihr Resultat.

**Die Daten zeigen einen signifikanten linearen Zusammenhang zwischen dem BMI und dem Korruptionsindex. Nimmt der BMI zu, sinkt der Korrupitionsindex. Mit $p$ = -0.9267 [-0.9758, -0.7891], p < .0001 ist der Zusammenhang sehr stark.**   
...
ggplot(corruption, aes(x = Median_BMI, y = Corruption_Perceptions_Index_2017_t)) +
  geom_point(color = "red", size = 2, alpha = .7) +
  geom_smooth(method = "lm", se = FALSE) +
  geom_text(aes(label = Country), check_overlap = TRUE)

cor.test(data = corruption, ~ Median_BMI + Corruption_Perceptions_Index_2017_t)
```
...
impfquote <- read_csv("../Datasets/impfquote_bag.csv")
...
5. Interpretiere dein Resultat: **Die Daten zeigen einen signifikanten linearen Zusammenhang zwischen kantonaler Impfquote und Covid-19 7-Tage-Inzidenz. Mit zunehmender Impfquote nimmt die Covid-19-Inzidenz ab ($p$ = -0.832 [-0.922  -0.657], p < 0.001)**   
